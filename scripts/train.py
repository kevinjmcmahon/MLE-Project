...
def run_training_pipeline(args):
    ...
    # --- THE KEY FIX: Re-introduce the HypertuneCallback ---
    ...
    callbacks = [
        ...
        HypertuneCallback() # Make sure the callback is included in the list
    ]
    ...
    if args.fine_tune_epochs > 0 and args.unfreeze_layers > 0:
        ...
        model.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=args.fine_tune_lr),
                      loss=tf.keras.losses.SparseCategoricalCrossentropy(),
                      metrics=['accuracy'])
        model.fit(train_ds, validation_data=validation_ds, epochs=args.fine_tune_epochs, 
